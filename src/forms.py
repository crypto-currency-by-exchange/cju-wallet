...
class SignUpForm(FlaskForm):
    ...
    passwd1 = PasswordField(
        "비밀번호", validators=[DataRequired("비밀번호는 반드시 입력해야 합니다")]
    )
    passwd2 = PasswordField(
        "비밀번호 확인", validators=[DataRequired("비밀번호는 반드시 입력해야 합니다")]
    )
    name = StringField(
        "이름(성 포함)",
    ...
    phone_mobile = StringField(
        ...
        validators=[
            DataRequired(
                "휴대전화 번호는 필수사항입니다. 향후 추가 인증을 위해 사용됩니다."
            )
        ],
    )
    email = EmailField(
        "이메일", validators=[DataRequired("이메일은 필수입력 사항입니다."), Email()]
    )
...
class TransferForm(FlaskForm):
    ...
    private_key = StringField(
        ...
        validators=[
            DataRequired("블록체인 전송을 위해서는 본인의 비밀키가 있어야 합니다.")
        ],
    )
    ...
    public_key = StringField(
        ...
        validators=[
            DataRequired("블록체인 전송을 위해서는 본인의 공개키가 있어야 합니다.")
        ],
    )
